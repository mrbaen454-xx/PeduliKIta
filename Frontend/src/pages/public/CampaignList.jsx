...
const CampaignList = () => {
  ...
  return (
    <div className="bg-background min-h-screen font-sans selection:bg-primary-fixed selection:text-on-primary-fixed pb-24">
      {/* Hero Section */}
      <section className="py-10 px-6 md:px-10 lg:px-12 max-w-[1240px] mx-auto bg-[var(--color-surface-container)]/70 backdrop-blur-2xl border border-[var(--color-primary)]/20 shadow-[0_20px_50px_rgba(0,107,44,0.12)] rounded-3xl mb-8 mt-4">
        <div className="flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-widest mb-6 bg-primary-fixed/20 px-3 py-1.5 rounded-full w-max border border-primary-fixed">
          <ShieldCheck className="w-4 h-4" />
          <span>Eksplorasi Inisiatif Kemanusiaan Terverifikasi</span>
        </div>
        
        <div className="flex flex-row justify-between items-start md:items-end gap-8">
           <div className="max-w-2xl">
             <h1 className="text-display-hero-mobile md:text-5xl font-serif font-bold leading-tight text-on-surface mb-4">
           ...
           <div className="bg-surface-container-lowest border border-outline-variant p-6 rounded-lg shadow-sm min-w-[280px]">
              <div className="flex justify-between items-center mb-4 text-xs font-bold text-outline uppercase tracking-wider">
                 ...
                 <span className="flex items-center gap-1 text-primary"><div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse"></div> Tahun 2026</span>
              </div>
      ...
      <section className="px-6 md:px-10 lg:px-12 py-8 max-w-[1240px] mx-auto mb-8 bg-[var(--color-surface-container)]/70 backdrop-blur-2xl border border-[var(--color-primary)]/20 shadow-[0_20px_50px_rgba(0,107,44,0.12)] rounded-3xl">
        <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-4 shadow-sm">
           ...
           <div className="flex flex-wrap gap-2 mb-4">
              <button 
                ...
                className={`px-4 py-2 rounded-full text-xs font-bold tracking-wide transition-colors ${selectedCategory === '' ? 'bg-primary text-white shadow-sm' : 'bg-surface-container-lowest border border-outline-variant text-on-surface-variant hover:bg-surface-container'}`}
              >
                Semua Kategori
              </button>
              {categories.map(c => (
                <button 
                  ...
                  className={`px-4 py-2 rounded-full text-xs font-bold tracking-wide transition-colors ${selectedCategory === c.name ? 'bg-primary text-white shadow-sm' : 'bg-surface-container-lowest border border-outline-variant text-on-surface-variant hover:bg-surface-container'}`}
                >
                  {c.name}
                </button>
              ))}
           ...
           <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4 border-t border-outline-variant/60">
              <div className="flex items-center gap-3">
                 <span className="text-xs font-bold text-outline">Status:</span>
                 ...
                 <div className="ml-4 flex items-center gap-2 text-xs font-bold text-primary bg-primary-fixed/20 px-3 py-1.5 rounded-full border border-primary-fixed">
                    <ShieldCheck className="w-3.5 h-3.5" /> Penyaluran diawasi langsung di 14 instansi
                 </div>
              </div>
              
              <div className="flex items-center gap-3 w-full sm:w-auto">
                 <span className="text-xs font-bold text-outline">Urutkan:</span>
                 <select 
                   className="bg-surface-container-low border border-outline-variant rounded px-3 py-2 text-xs font-bold text-on-surface outline-none focus:border-primary min-w-[200px]"
                   value={sortOption}
                   onChange={(e) => setSortOption(e.target.value)}
                 >
                   <option value="newest">Mendesak / Deadline Terdekat</option>
                   <option value="oldest">Terlama</option>
                   <option value="highest_target">Target Tertinggi</option>
                   <option value="lowest_target">Target Terendah</option>
                 </select>
              </div>
           </div>
        </div>
      ...
      {/* Grid Content */}
      <section className="px-6 md:px-10 lg:px-12 py-10 max-w-[1240px] mx-auto bg-[var(--color-surface-container)]/70 backdrop-blur-2xl border border-[var(--color-primary)]/20 shadow-[0_20px_50px_rgba(0,107,44,0.12)] rounded-3xl mb-8">
        {loading || forceState === 'loading' ? (
          ...
          <>
            ...
            {pagination && pagination.totalPages > 1 && (
              <div className="mt-12 flex justify-between items-center border-t border-outline-variant pt-6">
                 <span className="text-[13px] font-medium text-outline">Menampilkan pilihan <strong className="text-on-surface">1–{campaigns.length}</strong> dari <strong className="text-on-surface">{pagination.totalItems}</strong> kampanye</span>
      ...
      {/* Bottom CTA Panel */}
      <section className="max-w-[1240px] mx-auto px-6 md:px-10 lg:px-12 py-10 bg-[var(--color-surface-container)]/70 backdrop-blur-2xl border border-[var(--color-primary)]/20 shadow-[0_20px_50px_rgba(0,107,44,0.12)] rounded-3xl">
         <div className="bg-surface-container-low border border-outline-variant rounded-xl p-8 md:p-10 flex flex-row items-center justify-between gap-8">
            <div className="flex gap-6 items-start">
               <div className="w-12 h-12 rounded-lg bg-primary flex items-center justify-center text-white shrink-0 mt-1">
