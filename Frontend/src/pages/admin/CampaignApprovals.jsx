  ...
  return (
    <div className="w-full mx-auto min-h-screen">
      <div className="bg-surface-container-lowest rounded-xl shadow-sm border border-[var(--color-primary)]/20 p-6 md:p-8">
        <div className="mb-8">
          <h1 className="text-[28px] md:text-4xl font-bold text-[var(--color-inverse-surface)] font-serif mb-2">Persetujuan Kampanye</h1>
        <p className="text-[var(--color-on-surface-variant)]">Tinjau dan verifikasi kampanye baru sebelum dipublikasikan.</p>
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              ...
              <tbody className="divide-y divide-[var(--color-outline-variant)]/30">
                {campaigns.map((campaign) => (
                  <tr key={campaign.id} className="hover:bg-[var(--color-surface-container)]/30 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded bg-[var(--color-surface-container-high)] flex-shrink-0 overflow-hidden">
                          {campaign.image_url ? (
                        ...
                        <div>
                          <p className="font-bold text-[var(--color-inverse-surface)] line-clamp-1">{campaign.title}</p>
                          <p className="text-xs uppercase tracking-wider font-bold text-[var(--color-primary)] bg-[var(--color-primary)]/10 inline-block px-1.5 py-0.5 rounded mt-1">
                            {campaign.category?.name || 'Umum'}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="p-4 text-sm text-[var(--color-inverse-surface)] font-medium">
                      {campaign.campaigner?.name || 'Unknown'}
                    </td>
                    <td className="p-4">
                      <p className="font-bold font-mono text-[var(--color-primary)]">{formatCurrency(campaign.target_amount)}</p>
                    </td>
                    <td className="p-4">
                      <div className="flex items-center justify-center gap-2">
                        <Link 
                          to={`/admin/approvals/campaigns/${campaign.id}`} 
                          className="p-1.5 text-[var(--color-on-surface-variant)] hover:bg-[var(--color-surface-container-high)] rounded transition-colors"
                          title="Lihat Detail"
                        >
                          <Eye size={18} />
                        </Link>
                        <button 
                          ...
                          className="p-1.5 text-[#16A34A] hover:bg-[#16A34A]/10 rounded transition-colors"
                          title="Setujui"
                        >
                          <CheckCircle size={18} />
                        </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-surface-container-lowest rounded-xl shadow-xl w-full max-w-md border border-[var(--color-primary)]/20 overflow-hidden flex flex-col">
            <div className="p-4 border-b border-[var(--color-outline-variant)]/30 flex justify-between items-center bg-[var(--color-surface-container-lowest)]">
              <h3 className="font-bold text-lg text-[var(--color-error)] flex items-center gap-2">
                <AlertCircle size={20} /> Tolak Kampanye
              </h3>
              <button onClick={() => setIsRejectModalOpen(false)} className="text-[var(--color-on-surface-variant)] hover:text-[var(--color-error)] p-1 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleRejectSubmit} className="p-6">
              ...
              <div className="mb-6">
                <label className="block text-sm font-bold text-[var(--color-on-surface)] mb-2">Alasan Penolakan <span className="text-[var(--color-error)]">*</span></label>
                <textarea
                  required
                  value={rejectReason}
              ...
              <div className="flex justify-end gap-3">
                ...
                <button 
                  type="submit"
                  disabled={actionLoading}
                  className="px-4 py-2 text-sm font-bold bg-[var(--color-error)] text-white hover:bg-[#B91C1C] rounded-lg transition-colors disabled:opacity-50"
                >
